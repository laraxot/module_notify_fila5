...
./vendor/bin/phpstan analyse Modules/App/tests
...
```bash
# 1. Analisi
./vendor/bin/phpstan analyse Modules/App/tests > errors.txt
...
# 5. Commit
git add Modules/App/tests
git commit -m "fix(laraxot): phpstan level max compliance for tests"
```
...
4. ⏭️ **POI** - Iniziare Sprint 1: Correzione App test
5. ⏭️ **CONTINUO** - Aggiornare docs durante correzioni

### Decisioni da Prendere

- [ ] Confermare priorità moduli (App → User → Cms → Notify)
