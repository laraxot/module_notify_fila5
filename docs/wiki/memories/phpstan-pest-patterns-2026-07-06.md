---
title: "phpstan-pest-patterns-2026-07-06"
type: concept
tags: [deprecated]
created: 2026-07-14
updated: 2026-07-14
qmd: "phpstan-pest-patterns-2026-07-06 deprecated"
status: deprecated
related:
  - "./merge-collision-notify-lessons.md"
  - "./phpstan-pest-patterns-.md"
  - "./phpstan-pest-patterns-1.md"
  - "./phpstan-pest-patterns.md"
---

> Questo file è stato rinominato in [phpstan-pest-patterns-1.md](phpstan-pest-patterns-1.md). Non aggiungere date nel filename; usare `created/updated` nel front matter.
