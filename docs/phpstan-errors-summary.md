...
- **SushiToJsonsHelper rimosso**: Allineamento a `base_ptvx_fila5`. Il trait `SushiToJsonsHelper` non esiste in laraxot; `SushiToJsons` è self-contained. Rimossi da: `Section`, `PageContent`, `BaseModelJsons`. Risolto Fatal "Trait not found".
...
- `Livewire/TicketList.php`, `View/Components/Blocks/TicketList*.php`
  - Builder custom `Modules\App\Models\Builder` inesistente, proprietà non tipizzate, accesso a offset su `mixed`.
