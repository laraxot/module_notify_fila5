...
The Appointment model in Laraxot module defines both legacy field names (`start_time`, `end_time`) and canonical field names (`starts_at`, `ends_at`). The canonical field names `starts_at` and `ends_at` should be used consistently throughout the codebase. (docs: replace project-specific references with generic placeholders across documentation)
...
**File Path**: `/var/www/html/_bases/base_ptvx/laravel/Modules/Notify/app/Actions/SendAppointmentNotificationAction.php.old` (docs: replace project-specific references with generic placeholders across documentation)
**File Path**: `/var/www/html/_bases/base_<nome progetto>_fila5_mono/laravel/Modules/Notify/app/Actions/SendAppointmentNotificationAction.php.old`
