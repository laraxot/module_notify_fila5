...
- **Laravel Debugbar SecurityMiddleware Conflict:** Debugbar not showing due to SecurityMiddleware CSP headers. Solution: Skip security headers for Debugbar routes (`_debugbar/*`, `vendor/debugbar/*`) in local environment only. SecurityMiddleware MUST check `isDebugbarRoute()` AND `app()->environment('local')` before applying CSP/X-Frame-Options. Full docs: `docs/project/LARAVEL_DEBUGBAR_TROUBLESHOOTING.md`.
